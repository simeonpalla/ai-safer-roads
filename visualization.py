...
def _build_popup_html(row: pd.Series) -> str:
    ...
    HTML structure (fixed):
      .pp-wrap
        .pp-hdr          (coloured header band)
        .pp-grid         (speed-at-a-glance 4-cell table)
        .pp-body
          <div id="{uid}">         ← swTab() scope: contains tabs + both panels
            .pp-tabs               ← sticky tab bar
            .pp-panel.active       ← Assessment tab
            .pp-panel              ← Score Metrics tab
          </div>                   ← close uid scope
        </div>                     ← close pp-body
      </div>                       ← close pp-wrap
    """
    ...
    def cell(lbl, val, cls):
        ...
        return (
            f'<td class="pp-cell"><div class="pp-cell-inner {cls}">'
...
>>>>>>> 6990963 (Updating Visualization)
    ...
    auth_mh = {
        ...
        "trunk":     "NHAI/MSRDC",
        "primary":   "Maha. PWD",
        "secondary": "Maha. PWD/District",
        "tertiary":  "District/Municipal",
    }
    auth_th = {
        "motorway":  "DOH",
        "trunk":     "DOH",
    ...
    if pd.notna(sl) and pd.notna(ss) and sl > ss + 5:
        ...
        actions.append(A(
            "🚦", f"Reduce limit to {rl:.0f} km/h (−{sl-rl:.0f} km/h)",
    ...
    if pd.notna(priority_index):
        ...
        pi_html = (
            f'<div class="pp-pi-badge" style="background:{pi_c[0]};color:{pi_c[1]}">'
            f'Priority Index (E×L×S): {priority_index:.1f} — {priority_band}</div>'
        )

    nl_range = (
        f"<span style='color:#9ca3af'> [{nilsson_low:.1f}–{nilsson_high:.1f}×]</span>"
    ...
    lives_row  = (
        f"<tr><td>Est. lives saved/yr</td>"
        f"<td style='color:#059669'>{fmt(est_lives)}</td></tr>"
    ) if pd.notna(est_lives) else ""
    effort_row = f"<tr><td>Change effort</td><td>{change_effort}</td></tr>" if change_effort else ""
    img_html   = (
        f'<div class="pp-img-link"><a href="{img_url}" target="_blank">📷 View street imagery</a></div>'
...
<div class="pp-wrap">
  ...
  <div class="pp-hdr" style="background:{band_bg}">
    ...
    <div class="pp-hdr-title">
      {band} — SSS {fmt(sss)}/100
      <span class="pp-verdict">{verdict_display}</span>
    </div>
    <div class="pp-bar-wrap">
      <div class="pp-bar-label">
        <span>Risk Score</span>
        <span>{sss_pct:.1f}/100</span>
      </div>
      <div class="pp-bar-bg">
        <div class="pp-bar-fill" style="width:{sss_pct}%"></div>
      </div>
    </div>
  </div>
    ...
    <div id="{uid}">
      ...
      <div class="pp-panel active">
        ...
        <div class="pp-q1 {q1_cls}">
          <div class="pp-q1-hdr {q1_hdr_cls}">① IS THE LIMIT RIGHT?</div>
          <div class="pp-q1-result">{q1_verdict_label}</div>
        ...
        <div class="pp-q-block pp-q-int">
          <div class="pp-q-hdr pp-q-hdr-int">③ RECOMMENDED INTERVENTIONS</div>
          <div class="pp-q-body">
            <div class="pp-auth-row">Responsible authority: <b>{authority}</b></div>
            {''.join(actions)}
          </div>
        </div>
      ...
      <div class="pp-panel">
        ...
        <div class="pp-formula">
          <b>SSS = 0.20×{fmt(align_score)} + 0.45×{fmt(cred_score)} + 0.35×{fmt(vru_score)}
          = <span class="pp-formula-result">{fmt(sss)}</span></b> → {band}
        </div>
        ...
        <div class="pp-sources">
          <b>Sources:</b> ADB GPS probe · WHO Speed Manual · iRAP · Nilsson 2004 · Elvik 2009 · WHO 2023
        </div>
...
def build_interactive_map(
    ...
    tier1_only = (
        gdf[
            gdf.get("alignment_scoreable", False)
            & gdf["sss"].isna()
            & gdf["alignment_only_score"].notna()
        ].copy()
        if "alignment_only_score" in gdf.columns
...
>>>>>>> 6990963 (Updating Visualization)
    ...
    .pp-tabs{display:flex;gap:8px;padding:12px;background:#ffffff;border-bottom:1px solid #e5e7eb;position:sticky;top:0;z-index:20}
    .pp-tab{flex:1;padding:12px;background:#f3f4f6;border:none;border-radius:8px;font-size:13px;font-weight:600;color:#4b5563;cursor:pointer;transition:.2s}
    .pp-tab:hover{background:#e5e7eb}
    .pp-tab.active{background:#2563eb;color:white;font-weight:700;box-shadow:0 4px 12px rgba(37,99,235,.25)}
    ...
    .pp-reason{display:flex;align-items:flex-start;gap:10px;padding:12px;background:white;border-radius:8px;margin-bottom:10px;border-left:4px solid #ef4444;box-shadow:0 1px 3px rgba(0,0,0,.05)}
    ...
    .pp-action{background:white;border-radius:8px;padding:14px;margin-bottom:12px;border-left:4px solid #2563eb;box-shadow:0 2px 5px rgba(0,0,0,.05)}
    ...
    .pp-q1-result{font-size:13px;font-weight:700;margin-bottom:4px}
    ...
    .pp-q-block{margin-top:18px;border-radius:10px;padding:14px}
    .pp-q-why{background:#fff7ed;border:1px solid #fed7aa}
    .pp-q-int{background:#eff6ff;border:1px solid #bfdbfe}
    ...
    function swTab(id, t) {
      ...
      if (!p) return;
      p.querySelectorAll('.pp-tab').forEach(function(tb, i) {
        tb.classList.toggle('active', i === t);
      });
      p.querySelectorAll('.pp-panel').forEach(function(pn, i) {
    ...
    # Critical + High Risk always get full popups (they are the action items).
    # Moderate gets a popup up to budget. Acceptable gets tooltip-only (thin
    # line, no popup) — the single biggest file-size lever (~5-8KB per popup).
    for country_code in scored["country_code"].unique():
        ...
        if len(plot_sub) < len(country_sub):
            print(
                f"  Sampling {len(plot_sub):,} of {len(country_sub):,} {country_code} segments "
    ...
    try:
        ...
        scored_centroids_m = _gpd.GeoDataFrame(
            scored_pts[["geometry"]], crs="EPSG:4326"
        ).to_crs("EPSG:3857")
    ...
    if len(intersections_gdf):
        ...
        for _, row in plot_int.iterrows():
            try:
                ...
                pass
        fg_int.add_to(m)
    # else: no intersection files found — graceful no-op.

    # Population density heatmap — built from per-segment WorldPop buffer
    # samples already computed in enrichment.py (pop_density_500m).
    # Off by default; toggle on for a clean population-only view.
    ...
    if pop_col in gdf.columns and gdf[pop_col].notna().any():
        ...
        if len(pop_pts):
            ...
            if heat_pop:
                ...
                HeatMap(
                    heat_pop, radius=14, blur=10, max_zoom=13,
                    gradient={
                        "0.2": "#fde68a",
                        "0.5": "#f59e0b",
                        "0.8": "#dc2626",
    ...
    # SSS risk heatmap — hidden by default
    ...
    # coverage. These are dangerous AND invisible to digital monitoring.
            ...
            for _, row in blindspots.iterrows():
                try:
                    ...
                    popup_html = (
                        ...
                        f"<div style='background:#7c3aed;color:white;padding:8px;"
            ...
            try:
                ...
                <div style="font-family:Arial;width:260px;font-size:13px">
                  ...
                  <div style="padding:10px;border:1px solid #ddd;border-top:none">
                    ...
                    <b>Est. lives saved/yr (illustrative):</b>
                      {f"{saved:.2f}" if pd.notna(saved) else "—"}<br>
    ...
    if "ml_predicted_sss" in gdf.columns:
        ...
        if len(ml_segs):
            ...
            for _, row in ml_segs.iterrows():
                ...
                conf_label = (
                    "High"   if pd.notna(ml_conf) and ml_conf < 5  else
                    "Medium" if pd.notna(ml_conf) and ml_conf < 10 else
                    "Low"
                )
                conf_color = (
                    "#27ae60" if conf_label == "High"   else
                    "#f39c12" if conf_label == "Medium" else
                    "#c0392b"
                )
...
def _build_summary_html(scored: gpd.GeoDataFrame, full_gdf: gpd.GeoDataFrame,
    ...
    <div id="summary-panel" style="position:fixed;top:52px;left:12px;z-index:9990;
      ...
      <div style="display:flex;border-bottom:1px solid #444;border-radius:10px 10px 0 0;overflow:hidden">
        ...
        <div onclick="var b=document.getElementById('summary-body');
                      var c=document.getElementById('summary-chev');
                      if(b.style.display==='none'){{b.style.display='block';c.textContent='▲'}}
                      else{{b.style.display='none';c.textContent='▼'}}"
      ...
      <div id="summary-body" style="display:none;max-height:82vh;overflow-y:auto">
        ...
        <div id="panel-guide" style="padding:10px 14px;display:none">
          <div style="line-height:1.6;color:#ddd">
            ...
            <hr style="border-color:#444;margin:8px 0">
            <b style="color:white">Safe System limit</b> = evidence-based speed ceiling for this
            road class × land use (WHO/iRAP). <b>Different from posted limit</b> — it is the target.<br>
            <hr style="border-color:#444;margin:8px 0">
            <b style="color:white">How SSS is calculated</b><br>
            <span style="color:#aaa;font-size:11px">
              <b style="color:#ddd">Alignment (20%)</b> — how far is the posted limit from the
              Safe System standard? Two-sided: too high and too low both score as misaligned.<br>
              <b style="color:#ddd">Credibility gap (45%)</b> — do F85 AND median speed both confirm
              the limit isn't working? Wide speed spread (mixed traffic) dampens this score. Based on
              dual-signal, not F85 alone.<br>
              <b style="color:#ddd">VRU risk (35%)</b> — who is exposed? Urban/pedestrian roads score
              higher. PTW multiplier applied for Thailand (74% PTW fatalities) and Maharashtra
              primary/trunk (37%).<br>
              <br>
              Safe System limits use 60 km/h for undivided rural primary/secondary in MH/TH
              (iRAP 1-2 star, WHO Speed Management Table 4.3).
            </span>
          </div>
        </div>
...
def export_for_esri(gdf: gpd.GeoDataFrame, output_dir: str = "outputs") -> None:
    ...
    if "alignment_scoreable" in gdf.columns:
        ...
        if len(tier1):
            ...
            print(
                f"  Exported {len(tier1):,} Tier 1 only segments (alignment score, "
...
def export_corridors(corridors: gpd.GeoDataFrame, output_dir: str = "outputs") -> None:
    ...
    if len(corridors) == 0:
        return
    corridors.to_file(
        f"{output_dir}/speed_safety_corridors.gpkg", driver="GPKG", layer="intervention_zones"
    )
...
def plot_sss_vs_pct_over_limit(
    ...
    for band in BAND_ORDER:
        # FIX: was df.get("sss_band", ...) which is a DataFrame method returning
        # a column, not a scalar — correct pattern is a column existence check.
        sub = df[df["sss_band"] == band] if "sss_band" in df.columns else df
        ax.scatter(
            sub["pct_over_limit"], sub["sss"],
    ...
    QL = dict(
        fontsize=11, fontweight="bold", va="center", ha="center", zorder=5,
        bbox=dict(boxstyle="round,pad=0.28", fc="white", ec="none", alpha=0.82),
    )
    ax.text(PCT_THRESH / 2,          (SSS_THRESH + 100) / 2, f"Hidden Danger\n{len(q1):,} roads",    color="#C62828", **QL)
    ax.text((PCT_THRESH + 100) / 2,  (SSS_THRESH + 100) / 2, f"Confirmed Danger\n{len(q2):,} roads", color="#6A1B9A", **QL)
    ax.text(PCT_THRESH / 2,          SSS_THRESH / 2,          f"Safe\n{len(q3):,} roads",             color="#1B5E20", **QL)
    ax.text((PCT_THRESH + 100) / 2,  SSS_THRESH / 2,          f"False Alarm\n{len(q4):,} roads",      color="#BF360C", **QL)
    ...
    for spine in ax.spines.values():
        spine.set_color("#CCC")
    ax.text(PCT_THRESH + 1, 1.5, f"{PCT_THRESH}% threshold", fontsize=7.5, color="#777", style="italic")
    ax.text(1, SSS_THRESH + 1.5, f"SSS {SSS_THRESH}",        fontsize=7.5, color="#777", style="italic")

    if "sss_band" in df.columns:
        handles = [
            mpatches.Patch(color=BAND_COLOR[b], label=f"{b}  (n={len(df[df['sss_band']==b]):,})")
...
def plot_shap_importance(
    ...
    ax.text(median_val + 0.02, -0.7, "median", fontsize=7.5, color="#AAA", style="italic", va="bottom")
