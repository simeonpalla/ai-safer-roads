...
def _build_segment_row(r, rank: int) -> dict:
    ...
    osm_lit       = str(r.get("osm_lit",     pd.NA) or "")  if not pd.isna(r.get("osm_lit",     pd.NA)) else ""
    osm_surface   = str(r.get("osm_surface", pd.NA) or "")  if not pd.isna(r.get("osm_surface", pd.NA)) else ""
    blindspot     = bool(r.get("mapillary_blindspot", False) or False)
    ...
    if credibility == "Infrastructure-Forced":
        actions.append("Verify speed bump/table presence on-site before recommending limit change")
    ...
    # PTW-specific: flag high-risk road types where PTW fatality dominance documented
    ptw_flag_classes = {"secondary", "tertiary", "primary"}
    if road_class in ptw_flag_classes and country in ("TH", "Thailand"):
        actions.append("Deploy PTW-targeted enforcement / awareness campaign (TH: 74% of fatalities are PTW)")
    elif road_class in ("primary", "trunk") and country in ("MH", "India (Maharashtra)"):
        actions.append("PTW-truck conflict zone: physical separation or rumble strips recommended (MH: 37% PTW fatalities)")
    if not actions:
    ...
    try:
        ...
        method_rows = [
            ("Speed Safety Score (SSS)",
             ...
             "Combines: Safe System Alignment (20%), Limit Credibility Gap — dual-signal F85+median (45%), VRU Context Risk with PTW weighting (35%). "
             "Safe System limits use iRAP-calibrated 60 km/h ceiling for undivided rural primary/secondary roads in MH/TH."),
            ("SSS Band",
             "Critical ≥65 | High Risk 52–65 | Moderate 35–52 | Acceptable <35"),
            ("Priority Band",
